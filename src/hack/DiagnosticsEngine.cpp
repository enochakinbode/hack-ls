...
DiagnosticsMessagesMap DiagnosticsEngine::buildDiagnostics() {
  DiagnosticsMessagesMap uriToDiagnosticMessages;

  const std::map<std::string, Vector *> &diagnosticsMap =
  ...
  for (const auto &entry : diagnosticsMap) {
    ...
    for (int i = 0; i < diagnosticVector->size; i++) {
      auto *diagnostic = static_cast<Diagnostic *>(diagnosticVector->items[i]);
...
void DiagnosticsEngine::publishDiagnostics(
    const std::string &uri, const std::vector<DiagnosticMessage> &diagnostics) {
